...
public class BakeryServer {
    ...
    // 6) Handler for logout
    static class LogoutHandler implements HttpHandler {
        @Override
        public void handle(HttpExchange exchange) throws IOException {
            if ("POST".equals(exchange.getRequestMethod())) {
                currentSessionUserId = null;  // Invalidate session by clearing current session ID
    ...
    private static void sendResponse(HttpExchange exchange, int statusCode, String response) throws IOException {   
        ...
        exchange.getResponseHeaders().set("Access-Control-Allow-Headers", "Content-Type, Authorization");  // Allo these headers
